(function () {
    ...
    bbe("__resources").get(Context.resourceId, function (res, err) {
        if (res) {
            ...
            editor.setTheme("ace/theme/bos-backend");
    ...
    function update(showError) {
        ...
        try {
            ...
            bbe("__resources").put(Context.resourceId, settings, function (res, err) {
                if (err) {
                    ui.error("Error saving", err.message).effect("slide").closable();
                }
                if (!$("#notifications li").length) ui.notify("Saved").hide(1000).effect("slide");
            });
        } catch (ex) {
            if (showError) {
                ui.error("Invalid JSON", ex.message).effect("slide").closable();
            }
        }
    }
})();
