...
$(document).ready(function () {
    ...
    function loadResourceTypes() {
        bbe("__resources").get("types", function (res, err) {
    ...
    function loadResources() {
        bbe("__resources").get(function (res, err) {
    ...
    function createResource(typeId) {
        ...
        pathDialog("Create", "Create New " + (type.label || type.id), type.defaultPath || "/" + typeId.toLowerCase(), function (path) {
            bbe("__resources").post(
                ...
                function (res, err) {
                    if (err) {
                        return showError(err, "Could not create resource");
                    }
    ...
    function deleteResource(resource) {
        ui.confirm("Delete " + resource.id + "?", "This cannot be undone!")
            ...
            .show(function (ok) {
                if (ok) {
                    bbe("__resources").del(resource.id, function (res, err) {
                        if (err) {
                            return showError(err, "Could not delete resource");
                        }
                        loadResources();
    ...
    function renameResource(resource) {
        pathDialog("Rename", "Rename " + resource.id, "/" + resource.id, function (newPath) {
            ...
            resource.id = newPath;
            bbe("__resources").put(oldPath, resource, function (res, err) {
                if (err) {
                    return showError(err, "Could not rename resource");
                }
                loadResources();
