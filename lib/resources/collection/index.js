...
Collection.prototype.validate = function (body, create) {
    ...
    keys.forEach(function (key) {
        ...
        if (validation.exists(val)) {
            ...
            if (type === "date" && val != "null") val = new Date(val);

            if (val != "null" && !validation.isType(val, type)) {
...
Collection.prototype.sanitize = function (body) {
    ...
    keys.forEach(function (key) {
        ...
        if (expected == actual) {
            sanitized[key] = val;
        } else if (expected == "date" && val != "null" && (actual == "string" || actual == "number")) {
            sanitized[key] = new Date(val);
        } else if (expected == "date" && val == "null") {
            ...
            sanitized[key] = "" + val;
        } else if (val === null && (expected == "string" || expected == "array"))
            // keep null
            sanitized[key] = val;
    });
...
Collection.prototype.sanitizeQuery = function (query) {
    ...
    keys &&
        keys.forEach(function (key) {
            ...
            if (key === "$ignoreLimitRecursion") return;
            ...
            if (expected == "string" && actual == "number") {
                ...
                sanitized[key] = new Date(val);
            } else if (expected == "date" && actual == "object") {
                var keysFromObject = Object.keys(val);
...
Collection.prototype.handle = function (ctx) {
    ...
    if (ctx.req.method == "GET" && ctx.query.bosAction === "count") {
        delete ctx.query.bosAction;
    ...
    if (ctx.req.method == "GET" && ctx.query.id === "index-of") {
        delete ctx.query.id;
        var id = ctx.url.split("/").filter(function (p) {
            return p;
        })[1];
