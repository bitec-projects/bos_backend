...
(function () {
    ...
    vm.view.scrollToRow = function (row, fn) {
        ...
        setTimeout(function () {
            ...
            if (rowIndex === -1) {
                bbe(resource).get("index-of", rowId, function (res) {
                    var index = res.index,
        ...
        setTimeout(function () {
            ...
            if (type === "object" || type === "array") {
                ...
                try {
                    ...
                    try {
                        newVal = new Date(Number(newVal)).toISOString();
                    } catch (e) {
    ...
    function createRow(data) {
        ...
        rowVm._deleteRow = function () {
            if (rowVm.id()) {
                ...
                bbe(resource).del(rowVm.id(), { $skipEvents: true }, function (res, err) {
        ...
        rowVm._saveProp = function (prop, value, options) {
            ...
            if (lastValue !== value) {
                ...
                if (rowVm.id()) {
                    ...
                    bbe(resource).put(rowVm.id(), body, function (res, err) {
                        if (err) {
    ...
    function postRow(data, fn) {
        ...
        bbe(resource).post(data, function (res, err) {
    ...
    function getRowById(id) {
        return vm.data().filter(function (d) {
            return d.id() === id;
        })[0];
    }
    ...
    function loadProperties(fn) {
        bbe("__resources").get(resource, function (res, err) {
    ...
    function loadCount(fn) {
        bbe(resource).get({ bosAction: "count" }, function (res) {
            vm.count(res.count);
            if (typeof fn === "function") {
                fn();
            }
    ...
    function loadPage(page, fn) {
        ...
        bbe(resource).get({ $skip: begin * PAGE_SIZE, $limit: PAGE_SIZE * 3, $skipEvents: true }, function (res, err) {
            if (err) return;
            vm.currentPage(page);
            ko.mapping.fromJS({ data: res }, rowMapping, vm);
            if (vm.selectedRow() !== vm.newRow && vm.data().indexOf(vm.selectedRow()) === -1) {
                var newMappedRow = vm.data().filter(function (d) {
                    return vm.selectedRow() && d.id() === vm.selectedRow().id();
                })[0];
    ...
    loadProperties(function (props) {
        ...
        if (props) {
            ...
            bbe.on(resource + ":changed", function () {
                loadPage();
                loadCount();
            });
        } else {
