(function () {
    ...
    var propertyTypes = [
        ...
        },
        {
            id: "date",
            label: "date",
            tooltip: "Add a date property. If the incoming value is not a date string, or a numeric Unix time it will be rejected.",
            tooltipTitle: "ISO Date",
        },
        {
            id: "boolean",
            label: "boolean",
    ...
    function create() {
        ...
        var mapping = {
            properties: {
                ...
                create: _.bind(function (options) {
                    return createPropertyViewModel(options.data);
                }, this),
            },
        };
        ...
        function fetchProperties() {
            bbe("__resources").get(Context.resourceId, function (res, err) {
        ...
        bbe(Context.resourceId).get({ $limit: 1 }, function (res) {
            if (res.length) {
                vm.otherItems(true);
            } else {
                vm.otherItems(false);
            ...
            ko.computed(function () {
                ...
                propertiesArray.forEach(function (p) {
                    ...
                    if (p.name !== p.id) {
                        var rename = {};
                        rename[p.id] = p.name;
                        bbe(Context.resourceId).exec("rename", { properties: rename });
                        p.id = p.name;
                    }
                ...
                .extend({ throttle: 500 })
                .subscribe(function (propertiesJson) {
                    bbe("__resources").put(Context.resourceId, { properties: propertiesJson }, function (res, err) {
                        if (err)
                            return ui
                                .error("Error saving properties", err && err.message)
                                .sticky()
                                .effect("slide");
