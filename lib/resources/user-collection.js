...
UserCollection.prototype.handle = function (ctx) {
    ...
    if (ctx.req.method == "GET" && (ctx.url.indexOf("/index-of") === 0 || ctx.query.bosAction === "count")) {
        ...
        case "PUT":
            ...
            if (ctx.query.id || ctx.body.id) {
                ...
                this.store.first({ username: ctx.body.username }, function (err, u) {
                    if (u) return ctx.done({ errors: { username: "O usuário informado já está sendo utilizado." } });
                    uc.save(ctx, done);
                });
            }
            break;
...
UserCollection.prototype.setSessionId = function (ctx, sessionId) {
    // bbe internal client does not have res.cookies
...
UserCollection.prototype.handleLogin = function (ctx) {
    ...
    if (
        ...
        ctx.done("Nome de usário ou senha não informados.");
        return;
    }
    ...
    this.loginFindUser(ctx, function (err, user) {
        ...
        function loginFail(err) {
            checkAndSaveUser(function () {
                ...
                ctx.done("Usuário ou senha inválidos.");
            });
        }
