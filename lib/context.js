...
function Context(resource, req, res, server) {
    ...
    if (
        (this.query && typeof this.query.$ignoreLimitRecursion !== "undefined") ||
        (this.body && typeof this.body.$ignoreLimitRecursion !== "undefined")
    ) {
        var ignoreRecursionLimit = this.query.$ignoreLimitRecursion || this.body.$ignoreLimitRecursion || false;
        req.stack = req.stack || [];
        req.stack.ignoreRecursionLimit = ignoreRecursionLimit;
    }

    this.bbe = internalClient.build(server, req.session, req.stack, ctx);
}
    ...
    if (err) {
        ...
        try {
            ...
            if (status != 204 && status != 304) {
                if (body) {
                    this.res.setHeader("Content-Length", Buffer.isBuffer(body) ? body.length : Buffer.byteLength(body));
                }
