    ...
    baseMethods = {
        request: function (method, options, fn) {
            var promise = new Promise(function (resolve, reject) {
                var req, res, urlKey, recursions, recursionLimit;
                ...
                recursions = req.stack.filter(function (s) {
                    return s === urlKey;
                }).length;

                recursionLimit = (stack && stack.recursionLimit) || 2;

                // if (recursions < recursionLimit && (req.stack.ignoreRecursionLimit == undefined || (req.stack.ignoreRecursionLimit != undefined && req.stack.ignoreRecursionLimit==true))) {
    ...
    if (server.resources) {
        server.resources.forEach(function (r) {
            if (r.clientGeneration) {
                ...
                bbe[jsName] = createResourceClient(r, baseMethods);
            }
        });
    }
