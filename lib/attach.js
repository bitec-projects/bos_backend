...
/**
 * Attach bos backend router, sessions, db and functions into an existing http server instance.
 ...
 *
 *   var bosBackend = require('bos-backend')
 *   bosBackend.attach(server, {socketIo: io, env: ENV, db:{host:'localhost', port:27015, name:'my-db'} } );
...
function attach(httpServer, options) {
    ...
    server.options = options = _.extend(
        {
            db: { port: 27017, host: "127.0.0.1", name: "bos-backend" },
        },
        options
    );
        ...
        setupReqRes(server.options, req, res, function (err, next) {
            ...
            server.sessions.createSession(authToken || req.cookies.get("sid"), function (err, session) {
                if (err) {
                    ...
                    var root = req.headers["bbe-ssh-key"] || req.cookies.get("BbeSshKey");
