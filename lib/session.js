...
function SessionStore(namespace, db, sockets, options) {
    ...
    if (sockets) {
        if (this.subClient) {
            ...
            // need to resync
            this.subClient.subscribe("bosbackend#session#refreshrooms");
            this.subClient.subscribe("bosbackend#session#remove");
            this.subClient.on("message", function (channel, message) {
                var data;
                switch (channel) {
                    case "bosbackend#session#refreshrooms": // another node changed rooms for a session
                        ...
                        break;
                    case "bosbackend#session#remove": // another node removed a session
                        data = JSON.parse(message);
        ...
        sockets.on("connection", function (client) {
            ...
            function setSession(data) {
                if (!data || !data.sid || typeof data.sid !== "string") {
                    return;
                }
                var sid = data.sid;
...
SessionStore.prototype.cleanupInactiveSessions = function () {
    ...
    this.remove(
        {
            $or: [{ lastActive: { $lt: Date.now() - this.options.maxAge } }, { lastActive: { $exists: false } }],
        },
    ...
    function saveRooms() {
        self.save(function (err, data) {
            if (!err) {
                ...
                store.publish("bosbackend#session#refreshrooms", { id: self.id, sid: self.sid });
            }
        });
    }
...
Session.prototype.getUserChannel = function (uid) {
    if (uid || this.data.uid) {
        return "bbe_uid:" + (uid || this.data.uid);
    }
};
...
Session.prototype.remove = function (data, fn) {
    ...
    if (session.sid) {
        session.store.publish("bosbackend#session#remove", { id: session.store.id, sid: data.id });
    }
