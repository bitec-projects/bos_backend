(function (undefined) {
    ...
    function joinPath() {
        ...
        return (
            "/" +
            filterArray(paths, function (p, index) {
    ...
    function unwrapPromise(promise, fn) {
        return promise
            .then(function (res) {
                ...
                if (sessionId) {
                    window.bbe.setSessionId(sessionId);
                }
                return res;
            })
            .then(function (res) {
    ...
    var baseMethods = {
        get: function (options, fn) {
            ...
            return unwrapPromise(
                _bbe.ajax(root + joinPath(BASE_URL, options.path), {
                    method: "GET",
                    query: query,
                }),
                fn
        ...
        requestWithBody: function (method, options, fn) {
            ...
            return unwrapPromise(
                _bbe.ajax(root + joinPath(BASE_URL, options.path) + query, {
                    method: method,
    ...
    function parseGetSignature(args) {
        ...
        if ((args[i] !== consoleLog && typeof args[i] === "object") || !args[i]) {
            // IE considers console.log to be an object.
    ...
    function setBaseUrl(options) {
        ...
        if (typeof options === "string") {
            ...
            if (options.hostname) {
                ...
                if (!element) {
                    element = document.querySelector('script[src$="bbe.js"]');
                }
                if (element) {
                    var src = element.src || "";
                    var m = /((\w+:)?\/\/(.+):?(\d+)?)\//.exec(src);
    ...
    function checkAndConnectSocketIO() {
        if (!socket) {
            ...
            window.bbe.socket = socket;
            window.bbe.once("connect", function () {
                isSocketReady = true;
            });
            window.bbe.on("reconnect", function () {
                if (_sessionId) window.bbe.setSessionId(_sessionId, true);
            });
        }
    }

    window.bbe.setBaseUrl = setBaseUrl;
    window.bbe.getBaseUrl = getBaseUrl;

    window.bbe.setSessionId = function (sessionId, force) {
        if (force || sessionId != _sessionId) {
            window.bbe.socketReady(function () {
                window.bbe.socket.emit("server:setSession", { sid: sessionId });
                _sessionId = sessionId;
            });
        }
    };

    window.bbe.on = function () {
        checkAndConnectSocketIO();
        socket.on.apply(socket, arguments);
    };

    window.bbe.once = function (name, fn) {
        checkAndConnectSocketIO();
        var _fn = function () {
    ...
    window.bbe.off = function (name, fn) {
        checkAndConnectSocketIO();
        if (fn == null) {
    ...
    window.bbe.socketReady = function (fn) {
        checkAndConnectSocketIO();
        if (isSocketReady) {
            setTimeout(fn, 0);
        } else {
            window.bbe.once("connect", fn);
        }
    };
